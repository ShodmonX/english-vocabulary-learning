...
from app.db.repo.level_test import get_latest_confirmed_estimated_attempt
...
def _profile_text(user, summary: dict[str, object], estimated_attempt=None) -> str:
    username = f"@{user.username}" if user.username else t("common.none")
    total_seconds = int(summary["basic_remaining_seconds"]) + int(
        summary["topup_remaining_seconds"]
    )
    text = t(
        "profile.body",
    ...
    if estimated_attempt and estimated_attempt.level_estimate:
        text += "\n\n" + t(
            "profile.estimated_line",
            level=estimated_attempt.level_estimate,
        )
        text += "\n" + t(
            "profile.last_test_line",
            date=_format_local_date(estimated_attempt.updated_at),
        )
    return text
...
async def _render_profile(message: Message) -> None:
    async with AsyncSessionLocal() as session:
        ...
        estimated_attempt = await get_latest_confirmed_estimated_attempt(
            session,
            user_id=user.id,
            excluded_modes=("PLACEMENT_30",),
        )
    await message.answer(
        _profile_text(user, summary, estimated_attempt),
        reply_markup=profile_refresh_kb(),
    )
